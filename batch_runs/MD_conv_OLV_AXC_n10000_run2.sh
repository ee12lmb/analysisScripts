...
#     printf "Running step = $step | n = $n | repeat =  $repeat | seed = $seed ..."
     run="$IR $infile $step $n $repeat $seed $index MD_conv_OLV_AXC_n10000_run2.out/OLV_AXC_${index}_n${n}_r${repeat}_st${step}_sd${seed}.out > /dev/null"
